...
let gil_dot =
  if !Compileopt.use_fsm then
    Fsm.try_fsm Fsm.fsm_dot
  else
    Fsm.try_fst Fsm.fsm_dot

let do_compile is_sv_known out gr =
  do_phase "compiling to backend" (fun () -> Module_gen.do_compile is_sv_known backend out gr)

let do_compile_for_arrow out gr =
  do_phase "compiling to backend" (fun () -> Module_gen.do_compile_for_arrow backend out gr)
