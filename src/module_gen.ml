...
(* TODO:
    - get rid of Wadler_BE and generally clean up the code that handles backends.
...
let add_boilerplate gr = function
  | None -> ()
  | Some tx_code ->
      let boilerplate_shared =
        "let parse_file = Yak.Pami.Simple.parse_file parse\n\
         let parse_string = Yak.Pami.Simple.parse_string parse\n;;\n" in
      add_many_to_epilogue gr [ tx_code   ;
                                boilerplate_shared ]
...
let do_compile is_sv_known backend out gr =
    ...
    let post_parse_function = mk_post_parse_function gr in
    let tx_code_opt = match backend with
      | Wadler_BE ->
          gr.prologue <- [];
          gr.epilogue <- [];
      ...
      | Trans_BE ->
          print_prologue ();
          let tx_buf = print_gil_transducer is_sv_known out gr in
          let c = trans_bp gr tx_buf post_parse_function
            (if !Compileopt.infer_types_in_two_passes
             then mk_trans_bp_v1
             else mk_trans_bp_v2) in
          Some c in

    add_boilerplate gr tx_code_opt;
...
let do_compile_for_arrow backend out gr =
  ...
  let tx_code_opt = match backend with
    ...
    | Trans_BE ->
        print_prologue ();
        let tx_buf = maybe_wrap_in_module (fun out -> print_gil_transducer true out gr) out in
        let c = trans_bp gr tx_buf post_parse_function
          (if !Compileopt.infer_types_in_two_passes
           then mk_trans_bp_v1
           else mk_trans_bp_v2) in
